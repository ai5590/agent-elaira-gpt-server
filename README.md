...
2. Если `stepId == 1`, OpenAI вызывается без `previous_response_id`.
3. Если `stepId > 1`, сервис загружает из SQLite запись `(dialogId, stepId - 1)`.
4. Если предыдущий шаг не найден, API возвращает ошибку и прекращает обработку.
5. `openai_response_id` предыдущего шага используется как `previous_response_id` в первом запросе OpenAI для нового шага.
6. Ответ модели обязан быть строгим JSON-объектом с полями `stepDebug`, `dialogGoal` и `actions`.
7. Если первый ответ модели невалиден, сервис делает ровно один повторный запрос, используя внешний corrective prompt из `src/main/resources/prompts/retry-invalid-json.txt`.
8. Если и повторный ответ невалиден, API пишет критическую ошибку в лог, при необходимости отправляет уведомление в Telegram и возвращает ошибку клиенту.
9. Если JSON валиден, сервер считает токены, добавляет `dialogTotalTokens`, сохраняет итоговый шаг и возвращает ответ клиенту.
...
- Токены текущего шага = `request_tokens + response_tokens`
...
- `request_tokens` и `response_tokens` суммируются по обеим попыткам.
